...
export class dylib {
  public:
	...
	/**
	 *  This exception is raised when a library fails to load or a symbol fails
	 * to resolve
	 */
	 ...
	 *
	 *  @throws `dylib::load_error` if the library could not be opened
	 * (including the case of the library file not being found)
	...
	dylib(const char *dir_path, const char *lib_name,
		...
		if (!m_handle)
			throw load_error("Could not load library \"" + final_path +
	 ...
	 *  @throws `std::invalid_argument` if the argument or library handle is
	 * null
	 *
	...
	native_symbol_type get_symbol(const char *symbol_name) const {
		...
		if (!m_handle)
			throw std::logic_error(
				"The dynamic library handle is null. This object "
	 ...
	 */
	template <typename T> T *get_function(const char *symbol_name) const {
...
#pragma GCC diagnostic pop
	 ...
	 */
	template <typename T> T &get_variable(const char *symbol_name) const {
		return *reinterpret_cast<T *>(get_symbol(symbol_name));
	}
	 ...
	 *
	 *  @return true if the symbol exists in the dynamic library, false
	 * otherwise
	 */
	...
	static std::string get_error_description() noexcept {
		...
		const DWORD length =
			FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, nullptr, error_code,
						   lang, description, BUF_SIZE, nullptr);
		return (length == 0) ? "Unknown error (FormatMessage failed)"
							 : description;
#else
